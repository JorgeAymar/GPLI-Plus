...
export const rackSlots = pgTable(
  ...
  (table) => [index("rack_slots_rack_orientation_idx").on(table.rackAssetId, table.orientation)],
);
...
export const enclosureSlots = pgTable("enclosure_slots", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const clusterMembers = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.clusterAssetId, table.memberAssetId] })],
);
